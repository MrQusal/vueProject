        ...
        <table class="order-item" v-for="item in records" :key="item.id">
          ...
          <tbody>
            <tr v-for="(good, index) in item.orderDetailList" :key="good.id">
              ...
              <!-- 
                v-if="index === 0" 只需要第一个
                  原因：v-for 遍历 orderDetailList 时，表格需要合并单元格。
                       rowspan 的属性值动态等于 orderDetailList.length。
                       并且右边遍历了 orderDetailList.length 次，所以 td 会出现 orderDetailList.length 次
                       导致表格布局混乱
               -->
  ...
  export default {
    ...
    methods: {
      ...
      async getMyOrderList() {
        const res = await this.$API.reqMyOrders(this.pageNo, this.pageSize);
        if (res.code === 200) {
