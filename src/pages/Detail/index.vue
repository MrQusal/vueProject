        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAtrr in spuSaleAttrList" :key="spuSaleAtrr.id">
                <dt class="title">{{ spuSaleAtrr.saleAttrName }}</dt>
                <dd
                  changepirce="0"
                  :class="{ active: SaleAttrValue.isChecked === '1' }"
                  v-for="SaleAttrValue in spuSaleAtrr.spuSaleAttrValueList"
                  :key="SaleAttrValue.id"
                  @click="
                    changeAttrValue(
                      spuSaleAtrr.spuSaleAttrValueList,
                      SaleAttrValue
                    )
                  "
                >
...
export default {
  ...
  mounted() {
    ...
    this.getGoodDetails(skuId);
  },
  methods: {
    ...mapActions("detail", ["getGoodDetails", "addGoodtoShopCart"]),
    // 改变选中的版本/颜色（等属性）
    changeAttrValue(spuSaleAttrValueList, SaleAttrValue) {
    ...
    changeSkuNum(event) {
      ...
      // 小数
      else this.skuNum = Math.ceil(value);
    },
    // 添加到购物车
    addGoodToCar() {
      const { skuId } = this.$route.params;
      ...
      p.then(
        () => {
          ...
          // 为了让 addCartSuccess 组件具有 skuInfo（组件展示用），使用本地存储 sessionStorage
