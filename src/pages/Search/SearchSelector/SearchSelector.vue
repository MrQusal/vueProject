      ...
      <div class="fl value">
        <ul class="type-list">
          <li v-for="(value, index) in item.attrValueList" :key="index">
            <a @click="handlePropsClick(item, value)">{{ value }}</a>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    // 点击品牌名称，子-->父通信
    handleTradeClick(tarde) {
      this.$emit("getTradeInof", tarde);
    },
    // 点击不同的属性，子-->父通信
    handlePropsClick(attr, attrValue) {
      this.$emit("getPropsInfo", attr, attrValue);
    },
  },
};
</script>
